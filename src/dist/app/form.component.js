...
var $ = require("jquery");
var FormCanvasComponent = (function () {
    function FormCanvasComponent(winRef, selectedEle, services) {
        this.winRef = winRef;
        ...
        if (this.target.getAttribute('id') == "_domContainer" && req == "container_001") {
            ...
            if (req.split('_')[0] == "text" && this.target.tagName != "SPAN") {
                ...
                if (req == "input_001") {
                    ...
                    var textAtrea = document.createElement('textarea');
    ...
    FormCanvasComponent.prototype.ngOnDestroy = function () {
    ...
    FormCanvasComponent.prototype.ResizeEvent = function (dom) {
        var resizeClick = {};
        ...
        dom.addEventListener("mousedown", function (event) {
            ...
            var doc = document, docElem = doc.documentElement, body = document.body, win = window, clientTop = docElem.clientTop || document.body.clientTop || 0, clientLeft = docElem.clientLeft || body.clientLeft || 0, scrollTop = win.pageYOffset || jQuery.support.boxModel && docElem.scrollTop || body.scrollTop, scrollLeft = win.pageXOffset || jQuery.support.boxModel && docElem.scrollLeft || body.scrollLeft, top = box.top + scrollTop - clientTop, left = box.left + scrollLeft - clientLeft;
            ...
            if (l > box.width - 10 && l < box.width + 10 && t > box.height - 10 && t < box.height + 10) {
                ...
                if (ele.currentStyle) {
                    ...
                    try {
                        resizeClick.maxWidth = (e.maxWidth.replace(/[0-9]*\.?[0-9]*/g, "") == '%') ? (parseFloat(e.maxWidth.match(/[0-9]*\.?[0-9]*/g).join("")) * parentWidth / 100) : parseFloat(e.maxWidth.match(/[0-9]*\.?[0-9]*/g).join(""));
                    }
                    catch (err) {
                        resizeClick.maxWidth = parentWidth;
                    }
                    ;
                    try {
                        resizeClick.minWidth = (e.minWidth.replace(/[0-9]*\.?[0-9]*/g, "") == '%') ? (parseFloat(e.minWidth.match(/[0-9]*\.?[0-9]*/g).join("")) * parentWidth / 100) : parseFloat(e.minWidth.match(/[0-9]*\.?[0-9]*/g).join(""));
                    }
                    catch (err) {
                        resizeClick.minWidth = 0;
                    }
                    ;
                    try {
                        resizeClick.maxHeight = (e.maxHeight.replace(/[0-9]*\.?[0-9]*/g, "") == '%') ? (parseFloat(e.maxHeight.match(/[0-9]*\.?[0-9]*/g).join("")) * parentHeight / 100) : parseFloat(e.maxHeight.match(/[0-9]*\.?[0-9]*/g).join(""));
                    }
                    catch (err) {
                        resizeClick.maxHeight = parentHeight;
                    }
                    ;
                    try {
                        resizeClick.minHeight = (e.minHeight.replace(/[0-9]*\.?[0-9]*/g, "") == '%') ? (parseFloat(e.minHeight.match(/[0-9]*\.?[0-9]*/g).join("")) * parentHeight / 100) : parseFloat(e.minHeight.match(/[0-9]*\.?[0-9]*/g).join(""));
                    }
                    catch (err) {
                        resizeClick.minHeight = 0;
                    }
                    ;
                }
                resizeClick.top = top;
                resizeClick.left = left;
                if (isNaN(resizeClick.maxWidth)) {
                    resizeClick.maxWidth = parentWidth;
                }
                if (isNaN(resizeClick.minWidth)) {
                    resizeClick.minWidth = 0;
                }
                if (isNaN(resizeClick.maxHeight)) {
                    resizeClick.maxHeight = parentHeight;
                }
                if (isNaN(resizeClick.minHeight)) {
                    resizeClick.minHeight = 0;
                }
                resizeClick.parentHeight = parentHeight;
                resizeClick.parentWidth = parentWidth;
                var selected = selectedEle;
    ...
    FormCanvasComponent.prototype.updateView = function (dom) {
        this.selectedEle.setElement(dom);
        this.target.appendChild(dom);
        //console.log(this.selectedEle.getElement());
    };
    return FormCanvasComponent;
}());
__decorate([
    core_1.Output(),
    __metadata("design:type", core_1.EventEmitter)
], FormCanvasComponent.prototype, "response", void 0);
FormCanvasComponent = __decorate([
    core_1.Component({
    ...
    __metadata("design:paramtypes", [window_service_1.WindowRef, shared_service_1.SelectedElement, shared_services2_1.Services])
], FormCanvasComponent);
exports.FormCanvasComponent = FormCanvasComponent;
